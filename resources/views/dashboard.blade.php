<x-app-layout>
                    ...
                    <div class="p-6 text-gray-900">
                        ...
                        <div class="mt-4 p-4 border rounded-lg">
                            ...
                            @elseif($registration->status == 'Disetujui')
                                ...
                                <p class="text-sm text-gray-600">Selamat! Anda telah diterima sebagai anggota KOSUDGAMA. Informasi lebih lanjut akan kami kirimkan melalui email.</p>
                            @else
                                 <p class="text-lg font-bold text-red-600">{{ $registration->status }}</p>
                                 <p class="text-sm text-gray-600">Mohon maaf, pendaftaran Anda ditolak. Silakan hubungi administrasi untuk informasi lebih lanjut.</p>
                            @endif
                        </div>
                    </div>
                </div>
            @else
                {{-- TAMPILAN JIKA PENGGUNA BELUM MENGISI FORMULIR (LANGSUNG TAMPILKAN FORM) --}}
                @php
                    // Kode ini diperlukan untuk stylesheet & script formulir
                    // Jangan dihapus
                @endphp
                <style>
                ...
                <p class="mb-4 text-gray-600">Anda telah berhasil membuat akun. Langkah selanjutnya adalah melengkapi formulir aplikasi keanggotaan KOSUDGAMA di bawah ini.</p>
                
                <div class="form-container bg-white shadow-lg p-4 sm:p-6">
                    <div class="form-content">
                        {{-- SELURUH KODE FORMULIR DARI create.blade.php DITEMPEL DI SINI --}}
                        ...
                        <form action="{{ route('registration.store') }}" method="POST" enctype="multipart/form-data" id="registrationForm" class="mt-3 space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3"><div class="border-2 border-black"><div class="section-title">REFERENSI ANGGOTA (HARUS 2 ORANG)</div><div class="p-3"><p class="text-xs font-bold mb-2">Kami yang bertanda tangan di bawah ini:</p><div class="space-y-3"><div class="pl-4"><div class="input-group"><label for="nama_referensi_1" class="form-label w-24">1. Nama</label><input type="text" id="nama_referensi_1" name="nama_referensi_1" class="form-input" required value="{{ old('nama_referensi_1') }}"></div><div class="input-group mt-1"><label for="no_anggota_referensi_1" class="form-label w-24">No. Anggota</label><input type="text" id="no_anggota_referensi_1" name="no_anggota_referensi_1" class="form-input" required value="{{ old('no_anggota_referensi_1') }}"></div></div><div class="pl-4"><div class="input-group"><label for="nama_referensi_2" class="form-label w-24">2. Nama</label><input type="text" id="nama_referensi_2" name="nama_referensi_2" class="form-input" required value="{{ old('nama_referensi_2') }}"></div><div class="input-group mt-1"><label for="no_anggota_referensi_2" class="form-label w-24">No. Anggota</label><input type="text" id="no_anggota_referensi_2" name="no_anggota_referensi_2" class="form-input" required value="{{ old('no_anggota_referensi_2') }}"></div></div></div><p class="text-xs italic mt-3">Menyatakan mengetahui dan mengenal dengan baik calon anggota KOSUDGAMA yang mengajukan permohonan ini. "Apabila di kemudian hari calon anggota ini memiliki kewajiban yang harus diselesaikan dan atau dipenuhi kepada KOSUDGAMA, maka dalam hal tersebut kami ikut bertanggung jawab secara penuh."</p><div class="grid grid-cols-2 gap-4 mt-4 text-center"><div class="text-xs">Pemberi Referensi 1:<div class="mt-8 border-b border-dotted border-black">(.........................)</div></div><div class="text-xs">Pemberi Referensi 2:<div class="mt-8 border-b border-dotted border-black">(.........................)</div></div></div></div></div><div class="border-2 border-black"><div class="section-title">PERNYATAAN DAN KUASA</div><div class="p-3"><p class="text-xs italic">"Dengan mengetahui, memahami dan menyetujui segala persyaratan yang diberikan, kami selaku calon anggota mengajukan permohonan untuk menjadi anggota KOSUDGAMA. Apabila di kemudian hari terdapat hambatan dalam pemenuhan kewajiban dari saya selaku anggota, maka dengan ini memberikan kuasa kepada KOSUDGAMA untuk mengambil segala tindakan yang diperlukan sesuai dengan peraturan yang berlaku."</p><div class="text-center mt-4"><p class="text-xs">Yogyakarta, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}</p><div class="w-full h-24 my-2 bg-gray-100 border border-black relative"><canvas id="signature-canvas" class="w-full h-full"></canvas></div><div class="flex justify-center items-center"><span class="text-xs font-bold">( {{ Auth::user()->name }} )</span><button type="button" id="clear-signature" class="text-xs text-blue-600 hover:underline ml-2">Ulangi</button></div><input type="hidden" name="signature" id="signature-input"></div></div></div></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (canvas) { // Pastikan canvas ada sebelum menjalankan script
